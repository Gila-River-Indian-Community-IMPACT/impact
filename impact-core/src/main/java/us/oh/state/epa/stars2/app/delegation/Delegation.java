package us.oh.state.epa.stars2.app.delegation;
...
public class Delegation extends TaskBase  implements IAttachmentListener {
    ...
    public String findOutcome(String url, String ret) {
    	...
    	applicationDetail.setApplicationID(delegationRequest.getApplicationID());
        return "applicationDetail";
    }
    
    @Override
    public String getExternalNum() {
        return getDelegationRequest().getApplicationNumber();
    }

    @Override
    public Integer getExternalId() {
        return getDelegationRequest().getApplicationID();
    }

    @Override
    public void setExternalId(Integer externalId) {
        setDelegationRequestID(externalId);
    ...
    public List<ValidationMessage> validate(Integer inActivityTemplateId) {
        ...
        if(462 == inActivityTemplateId.intValue()){
        	boolean issued = false;
        	//Delegation not Issued.
        	GeneralIssuance gi = 
                (GeneralIssuance)FacesUtil.getManagedBean("generalIssuance");
        	gi.loadIssuances(delegationRequest);
        	
        	if(gi.getIssuances() != null){
        		for(GenericIssuance issuance : gi.getIssuances()){
        	...
        	if(!issued){
        		messages.add(new ValidationMessage("Application",
        				"Cannot complete task, Issuance not Completed.",
    ...
    public String toExternal() {
    	...
    	applicationDetail.setApplicationID(delegationRequest.getApplicationID());
    	setFromTODOList(true);// set to true only when navigated from workflow tab.
        return "applicationDetail";
        ...
        public void setDelegationRequest(DelegationRequest delegationRequest) {
            ...
            setFromTODOList(false);// see toExternal();
        ...
        */
        ...
        public final String save() throws RemoteException{
        	ValidationMessage[] validationMessages;
        	if (delegationRequest.isNewRecord()) {
        		//logger.debug("new record...");
        		validationMessages = getDelegationRequestService().validateDelegationRequest(delegationRequest);
        	...
        	initializeAttachmentBean();
        	refresh();
        	return SUCCESS;    //return to the same page
        }
