...
import us.oh.state.epa.stars2.database.dao.ApplicationEUTypeDAOLUD;
...
@Repository
public class ApplicationEUTypeSQLDAOLUD extends ApplicationEUTypeSQLDAO
		implements ApplicationEUTypeDAOLUD {

	@Override
	public NSRApplicationEUType createApplicationEUType(
			NSRApplicationEUType euType) throws DAOException {
		if(euType instanceof NSRApplicationEUTypeLUD) {
			NSRApplicationEUTypeLUD appEuType = (NSRApplicationEUTypeLUD) euType;
			NSRApplicationEUTypeLUD ret = appEuType;
	
			checkNull(appEuType.getApplicationEuId());
	
			ConnectionHandler connHandler = new ConnectionHandler(
					"ApplicationEUTypeSQL.createApplicationEUTypeLUD", false);
	
			int i = 1;
			connHandler.setInteger(i++, appEuType.getApplicationEuId());
			connHandler.setInteger(i++, appEuType.getMaxHourlyThroughput());
	...
	public NSRApplicationEUType retrieveApplicationEUType(Integer appEuId)
			throws DAOException {
		NSRApplicationEUTypeLUD ret = new NSRApplicationEUTypeLUD();
		...
		if (appEuId != null) {
			try {
				...
				pStmt = conn
						.prepareStatement(loadSQL("ApplicationEUTypeSQL.retrieveApplicationEUTypeLUD"));
				...
				if (rs.next()) {
					ret.populate(rs);
					ret.setEmissionUnitTypeCd(EmissionUnitTypeDef.LUD);
				}
	...
	public void removeApplicationEUType(Integer appEuId) throws DAOException {
		ConnectionHandler connHandler = new ConnectionHandler(
				"ApplicationEUTypeSQL.removeApplicationEUTypeLUD", false);
