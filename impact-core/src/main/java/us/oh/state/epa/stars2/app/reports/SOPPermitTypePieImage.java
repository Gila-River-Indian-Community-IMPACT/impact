...
public class SOPPermitTypePieImage extends ReportImageBase implements Image {
    public SOPPermitTypePieImage() {
        super();
    }
    ...
    public final void draw(int width, int height) throws Exception {
        ...
        for (PermitSOPData sopData : details) {
            SimpleIdDef sd = sdMap.get(sopData.getPermitType());
            if (sd == null) {
                sd = new SimpleIdDef();
                sd.setId(1);
                sd.setDescription(sopData.getPermitType());
                sdMap.put(sopData.getPermitType(), sd);
            } else {
                sd.setId(sd.getId() + 1);
            }
        }
