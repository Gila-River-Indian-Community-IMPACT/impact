...
public class PerReportingPeriod extends AppBase {
    ...
    public CompliancePerDetail[] retrieveEusForPeriod() {
        if (perDetail.size()==0) {
            Hashtable<Integer,String> uniqueEU = new Hashtable<Integer,String>();
                ...
                for (int i=0;i<permits.size();i++) {
                    ...
                    for (int j=0;j<eug.size();j++) {
                        ...
                        for (int k=0;k<eus.size();k++) {
                            ...
                            if (!uniqueEU.containsKey(new Integer(eu.getCorrEpaEMUID()))) {
                                logger.warn("Building EU's for period: Adding EU #"+eu.getCorrEpaEMUID());
                                CompliancePerDetail dTemp = new CompliancePerDetail();
                                dTemp.setEUId(eu.getCorrEpaEMUID());
                                dTemp.setPermitId(p.getPermitID());
                                perDetail.add(dTemp);
                                uniqueEU.put(new Integer(eu.getCorrEpaEMUID()),"");
                            }
                        }
