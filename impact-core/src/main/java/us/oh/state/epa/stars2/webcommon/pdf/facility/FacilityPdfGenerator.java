package us.oh.state.epa.stars2.webcommon.pdf.facility;
...
public class FacilityPdfGenerator extends PdfGeneratorBase {
	...
	private static Logger logger = Logger.getLogger(FacilityPdfGenerator.class);
	 ...
	 * 
	 */
	public FacilityPdfGenerator() throws DocumentException {
		try {
	...
	public void generatePdf(Facility facility, OutputStream os, String titleSuffix)
		...
		String heading = "Facility Detail Report" + titleSuffix;
	...
	private void printFacility() throws IOException, DocumentException {
		...
		sections.add(generateFacilityData());
		sections.add(generateLocation());

		if (facility.isHasApis()) {
			sections.add(generateApis());
		}

		if (CompMgr.getAppName().equals(CommonConst.INTERNAL_APP)) {
			sections.add(generateNotes(facility.getNotes()));
		}
		sections.add(generateNaicsCodes());
		
		if (!CompMgr.getAppName().equals(CommonConst.PUBLIC_APP) && FacilityTypeDef.hasHCAnalysis(facility.getFacilityTypeCd())){
			sections.add(generateExtendedHydrocarbonAnalysisTable(facility));
			sections.add(generateHydrocarbonAnalysisSampleDetailTable(facility));
			sections.add(generateHydrocarbonAnalysisDecanePropertiesTable(facility));
					sections.add(generateHydrocarbonAnalysisOtherParts(facility));
		}
		...
		if (!CompMgr.getAppName().equals(CommonConst.PUBLIC_APP)) {
			sections.add(generateFedRulesAppl());
		}
		if (CompMgr.getAppName().equals(CommonConst.INTERNAL_APP)) {
			sections.add(generateFacAttachments());
		}

		sections.add(generateVersion());

		document.add(sections);

		for (EmissionUnit eu : facility.getEmissionUnits()) {
			// Mantis 2499 - exclude invalid EUs from PDF file
			if (!EuOperatingStatusDef.IV.equals(eu.getOperatingStatusCd())) {
				generateEmissionUnit(eu);
			}
		}

		for (ControlEquipment ce : facility.getControlEquips()) {
			generateControlEquipment(ce);
		}

		for (EgressPoint egp : facility.getEgressPoints()) {
			generateEgressPoint(egp);
		}

		for (ContinuousMonitor monitor : facility.getContinuousMonitorList()) {
			generateCemComMonitor(monitor);
		}

		for (FacilityCemComLimit limit : facility.getFacilityCemComLimitList()) {
			generateCemComLimit(limit);
		}
...
//		sections.add(generateCEMData(egp));
	...
	private ListItem generateFacilityData() throws DocumentException {
		...
		if (CompMgr.getAppName().equals(CommonConst.INTERNAL_APP)) {
			temp = toString(facility.isAdministrativeHold());
			if (facility.isAdministrativeHold()){
				generate2Columns(table, "Administrative Hold:", temp, true, "Administrative Hold Active", null, false); 
			} else {
				generate2Columns(table, "Administrative Hold:", temp, true, null, null, false); 
			}
		}
	...
	private ListItem generateLocation() throws DocumentException {
		...
		for (Address address : facility.getAddressesList()) {
			if (address.getFullAddress() == null) {
				table = generateSubTable("Location Detail :");
			} else {
				table = generateSubTable("Location Detail For : " + address.getFullAddress());
			}
			item1.add(table);
			...
			generate2Columns(table, "Quarter Quarter:", address.getQuarterQuarter(), false, 
					"Quarter:", address.getQuarter(), false);
			generate2Columns(table, "Section:", address.getSection(), false, null, null, false);
			generate2Columns(table, "Township:", address.getTownship(), false, "Range:", address.getRange(), false);

			temp = County.getData().getItems().getItemDesc(address.getCountyCd());
			temp1 = State.getData().getItems().getItemDesc(address.getState());
			generate2Columns(table, "County:", temp, false, "State:", temp1, false);

			if (!SystemPropertyDef.getSystemPropertyValueAsBoolean("hideDistrict", false)) {
				temp = District.getData().getItems().getItemDesc(address.getDistrictCd());
				generate2Columns(table, "District:", temp, false, null, null, false);
			}

			temp = IndianReservationDef.getData().getItems().getItemDesc(address.getIndianReservationCd());
			generate2Columns(table, "Indian Reservation:", temp, false, null, null, false);
			generate2Columns(table, "Physical Address 1:", address.getAddressLine1(), false,
					"Physical Address 2:", address.getAddressLine2(), false);
			generate2Columns(table, "City:", address.getCityName(), false, "Zip:", address.getZipCode(), false);
			
			temp = address.getBeginDate() == null ? null : dateFormat1.format(address.getBeginDate());
			generate2Columns(table, "Effective Date:", temp, false, null, null, false);
	...
	private ListItem generateContacts() throws DocumentException {
		...
		for (Contact tempCont : allContacts) {
			...
			temp = tempCont.getTitleCd() == null ? null : contactTitles.get(tempCont.getTitleCd());
			generate2Columns(table, "Title:", temp, false, "First Name:", tempCont.getFirstNm(), false);
			generate2Columns(table, "Middle Name:", tempCont.getMiddleNm(), false, 
					"Last Name:", tempCont.getLastNm(), false);
			generate2Columns(table, "Suffix:", tempCont.getSuffixCd(), false, null, null, false);
			generate2Columns(table, "Company Title:", tempCont.getCompanyTitle(), false, 
					"Contact's Company Name:", tempCont.getCompanyName(), false);
			generate2Columns(table, "", "", false, "", "", false);

			generate2Columns(table, "Address 1:", tempCont.getAddress().getAddressLine1(), false, null, null, false);
			generate2Columns(table, "Address 2:", tempCont.getAddress().getAddressLine2(), false, null, null, false);
			generate2Columns(table, "City:", tempCont.getAddress().getCityName(), false, 
					"Zip Code:", tempCont.getAddress().getZipCode(), false);
			temp = State.getData().getItems().getItemDesc(tempCont.getAddress().getState());
			generate2Columns(table, "State:", temp, false, null, null, false);
			generate2Columns(table, "", "", false, "", "", false);

			temp = phoneConvert.tryFormatPhoneNumber(tempCont.getPhoneNo());
			generate2Columns(table, "Primary Phone No.:", temp, false, 
					"Primary Ext. No.:", tempCont.getPhoneExtensionVal(), false);
			temp = phoneConvert.tryFormatPhoneNumber(tempCont.getSecondaryPhoneNo());
			generate2Columns(table, "Secondary Phone No.:", temp, false, 
					"Secondary Ext. No.:", tempCont.getSecondaryExtensionVal(), false);
			temp = phoneConvert.tryFormatPhoneNumber(tempCont.getMobilePhoneNo());
			generate2Columns(table, "Mobile Phone No.:", temp, false, null, null, false);
			temp = phoneConvert.tryFormatPhoneNumber(tempCont.getFaxNo());
			generate2Columns(table, "Fax No:", temp, false, null, null, false);
			generate2Columns(table, "", "", false, "", "", false);
	...
	private ListItem generateEmissionUnitData(EmissionUnit eu) throws DocumentException {
		...
		if (eu.getEmissionUnitTypeCd().equals(EmissionUnitTypeDef.ABS)) {
			...
			if (!Utility.isNullOrEmpty(temp)) {
				if (temp.equalsIgnoreCase("Other")) {
					...
					generate2Columns(table, "Substrate being Removed (Other):", temp, false, null, null, false);
				}
			}
			...
			if (!Utility.isNullOrEmpty(temp)) {
				if (temp.equalsIgnoreCase("Leaded Paint")) {
					...
					generate2Columns(table, "Percentage Concentration Of Lead in Paint:", temp, false, null, null,
							false);
				}
			}
			...
			temp = toString(eu.getEmissionUnitType().getMaxNumberOfTimesBlastMediaReclaimedForReuse());
			generate2Columns(table, "Maximum Number Of Times Blast Media Reclaimed For Reuse:", temp,
					false, null, null, false);

			
			temp = AbsApplicationMethodTypeDef.getData().getItems()
					.getItemDesc(eu.getEmissionUnitType().getApplicationMethod());
			generate2Columns(table, "Application Method:", temp, false, null,
					null, false);
			...
			if (!Utility.isNullOrEmpty(temp)) {
				if (temp.equalsIgnoreCase("Other")) {
					...
					generate2Columns(table, "Type of Material Being Coated (Other):", temp, false, null, null, false);
				}
			}
			...
			if (!Utility.isNullOrEmpty(temp)) {
				if (temp.equalsIgnoreCase("Other")) {
					...
					generate2Columns(table, "Application Method (Other):", temp, false, null, null, false);
				}
			}
			...
			temp = EngTypeOfUseDef.getData().getItems().getItemDesc(eu.getEmissionUnitType().getTypeOfUse());
			generate2Columns(table, "Type Of Use:", temp, false, null, null, false);
			...
			if (!Utility.isNullOrEmpty(temp)) {
				if (temp.equalsIgnoreCase("Other")) {
					...
					generate2Columns(table, "Unit Type (Other):", temp, false, null, null, false);
				}
			}
			...
			//FUG type data
			temp = toString(eu.getEmissionUnitType().isFugitiveLeaks());
			generate2Columns(table, "Fugitive Emission Type - Fugitive Leaks at Oil and Gas Sites:", temp, true, null, null, false);
			if (eu.getEmissionUnitType().isFugitiveLeaks()){
				fugComponentTable=generateFugitiveComponentTable(eu);
			}
			...
			if (!Utility.isNullOrEmpty(temp)) {
				if (temp.equalsIgnoreCase("Other")) {
					...
					generate2Columns(table, "Type of Explosive (Other):", temp, false, null, null, false);
				}
			}
			...
			if (EquipTypeDef.INTERMITTENT.equals(eu.getEmissionUnitType().getEquipmentType())){
				temp = toString(eu.getEmissionUnitType().getBleedRate());
				generate2Columns(table, "Bleed Rate per Unit (cu. ft/hr):", temp, false, null, null, false);
			}
			
			if (EquipTypeDef.PUMP.equals(eu.getEmissionUnitType().getEquipmentType())){
				temp = toString(eu.getEmissionUnitType().getGasConsumptionRate());
				generate2Columns(table, "Gas Consumption Rate per Unit (cu. ft/hr):", temp, false, null, null, false);	
			}

			temp = toString(eu.getEmissionUnitType().getEqptCount());
			generate2Columns(table, "Total Count of Equipment:", temp, false, null, null, false);
			...
			if (!Utility.isNullOrEmpty(temp)) {
				if (temp.equalsIgnoreCase("Other")) {
					...
					generate2Columns(table, "Press Type (Other):", temp, false, null, null, false);
				}
			}
			...
			if (!Utility.isNullOrEmpty(temp)) {
				if (temp.equalsIgnoreCase("Other")) {
					...
					generate2Columns(table, "Type of Contaminant being Treated (Other):", temp, false, null, null,
							false);
				}
			}
			...
			if (!Utility.isNullOrEmpty(eu.getEmissionUnitType().getEquipmentType())) {
				if (eu.getEmissionUnitType().getEquipmentType().equalsIgnoreCase("other")) {
					temp = eu.getEmissionUnitType().getEquipmentTypeOther();
					generate2Columns(table, "Equipment Type (Other):", temp, false, null, null, false);
				}
			}
			...
			if (eu.getEmissionUnitType().getPlatingLineFlag() != null) {
				temp = eu.getEmissionUnitType().getPlatingLineFlag().equalsIgnoreCase("Y") ? "Yes"
						: "No";
			generate2Columns(table, "Is this for a Plating Line?:", temp, false, null,
					null, false);
			}
			
			
			if (eu.getEmissionUnitType().getPlatingLineFlag() != null && temp.equalsIgnoreCase("Yes") ) {
				temp = TkoPlatingTypeDef.getData().getItems().getItemDesc((eu.getEmissionUnitType().getPlatingType()));
				generate2Columns(table, "Type of Plating:", temp, false, null, null, false);

				if (!Utility.isNullOrEmpty(temp)){
					if (temp.equalsIgnoreCase("Other")) {
						temp = toString(eu.getEmissionUnitType().getPlatingTypeOther());
						generate2Columns(table, "Type of Plating (Other):", temp, false, null, null, false);
					}
				}
			}
			...
			if (!Utility.isNullOrEmpty(temp)) {
				if (temp.equalsIgnoreCase("Other")) {
					...
					generate2Columns(table, "Type of Metal (Other):", temp, false, null, null, false);
				}
			}
			
			temp = toString(eu.getEmissionUnitType().getCapacity());
			generate2Columns(table, "Tank Capacity (gals):", temp, false,
					null, null, false);	
			...
			if (temp != null) {
				if (temp.equals(TnkMaterialStoredTypeDef.LIQUID)) {
					temp = TnkMaterialStoredTypeLiquidDef.getData().getItems()
							.getItemDesc(eu.getEmissionUnitType().getLiquidMaterialTypeStored());
					generate2Columns(table, "Type of Liquid:", temp, false, null, null, false);
					
					if (temp != null) {
						if (temp.equals("Other Liquid")) {
			...
			if (eu.getEmissionUnitType().getSubmergedFillPipeFlag() != null) {
				temp = eu.getEmissionUnitType().getSubmergedFillPipeFlag().equals("Y") ? "Yes"
						: "No";
			generate2Columns(table, "Submerged Fill Pipe?:", temp, false, null,
					null, false);
			}
			...
			temp = TnkTankLocationDef.getData().getItems()
					.getItemDesc((eu.getEmissionUnitType().getTankLocation()));
			generate2Columns(table, "Tank Location:", temp,
					false, null, null, false);
			...
			if (powerRatingFlag != null) {
				if (powerRatingFlag.equalsIgnoreCase("Y")) {
					powerRatingFlagStr = "Yes";
				} else if (powerRatingFlag.equalsIgnoreCase("N")) {
					powerRatingFlagStr = "No";
				}
				generate2Columns(table, "Power Rating (hp) greater than 5:", powerRatingFlagStr, false, null, null,
						false);
			}
		...
		if (fugComponentTable!=null){
			item1.add(fugComponentTable);
		}
		...
		generate2Columns(table2, "AQD Description:", eu.getEuDesc(), false, null, null, false);
		generate2Columns(table2, "Company Equipment ID:", eu.getCompanyId(), false, null, null, false);
		generate2Columns(table2, "Company Equipment Description:", eu.getRegulatedUserDsc(), false, null, null, false);
		...
		if (eu.getOperatingStatusCd().equals(OperatingStatusDef.SD)) {
			...
			generate2Columns(table2, "Operating Status:", temp, false, "Shutdown Date:", temp1, false);
			if (facility.getPermitClassCd() != null && facility.getPermitClassCd().equals(PermitClassDef.TV)) {
				...
				generate2Columns(table2, "Shutdown Notification Date:", temp1, false, null, null, false);
			}
		} else {
			generate2Columns(table2, "Operating Status:", temp, false, null, null, false);
		}

		generate2Columns(table2, "", "", false, "", "", false);

		temp = toDateFormat1(eu.getEuInitInstallDate());
		generate2Columns(table2, "Initial Construction Commencement Date:", temp, false, null, null, false);
		...
		generate2Columns(table2, "Most Recent Construction/Modification Commencement Date:", temp, false, null, null,
				false);
		temp = toDateFormat1(eu.getEuStartupDate());
		generate2Columns(table2, "Most Recent Operation Commencement Date:", temp, false, null, null, false);
...
//		sections.add(generateCEMData(egp));
	...
	public static void main(String[] args) {
		...
		try {
			...
			Facility facility = facilityBO.retrieveFacilityProfile(48516,
					false, null);
			FacilityPdfGenerator gen = new FacilityPdfGenerator();
	...
	private Table generateFugitiveComponentTable(EmissionUnit eu)
			throws DocumentException {
		Cell cell;
		String temp;

		Table table = new Table(5);
		float[] widths = { 35, 30, 30, 30, 35};
		table.setWidths(widths);
		table.setPadding(2);
		table.setAlignment(Element.ALIGN_LEFT);
		
		//Table heading
		Cell tableHeading = createCell("Total Count of Component");
		tableHeading.setHorizontalAlignment(Element.ALIGN_CENTER);
		tableHeading.setBorder(0);
		tableHeading.setColspan(5);
		table.addCell(tableHeading);
		cell = createCell(defaultLeading, "Component", normal1Font);
		table.addCell(cell);
		cell = createCell(defaultLeading, "Gas", normal1Font);
		table.addCell(cell);
		cell = createCell(defaultLeading, "Heavy Oil (<= 20\u00B0 API)", normal1Font); // this font does not handle "<=" unicode escape
		table.addCell(cell);
		cell = createCell(defaultLeading, "Light Oil(> 20\u00B0 API)", normal1Font);
		table.addCell(cell);
		cell = createCell(defaultLeading, "Water/Condensate", normal1Font);
		table.addCell(cell);
		
		for(Component component : eu.getEmissionUnitType().getComponents() ){
			temp = FugitiveComponentDef.getData().getItems().getItemDesc(component.getComponentCd());
			cell = createCell(defaultLeading, temp, data1Font);
			table.addCell(cell);

			String gas = null;
			if (component.getGas() != null) {
				gas = formatFugitiveComponent(component.getGas());
			}
			cell = createCell(defaultLeading, gas, data1Font);
			table.addCell(cell);

			String heavyOil = null;
			if (component.getHeavyOil() != null) {
				heavyOil = formatFugitiveComponent(component.getHeavyOil());
			}
			cell = createCell(defaultLeading, heavyOil, data1Font);
			table.addCell(cell);

			String lightOil = null;
			if (component.getLightOil() != null) {
				lightOil = formatFugitiveComponent(component.getLightOil());
			}
			cell = createCell(defaultLeading, lightOil, data1Font);
			table.addCell(cell);

			String water = null;
			if (component.getWater() != null) {
				water = formatFugitiveComponent(component.getWater());
			}
			cell = createCell(defaultLeading, water, data1Font);
			table.addCell(cell);
		}
		return table;
	}
	...
	private ListItem generateExtendedHydrocarbonAnalysisTable(Facility facility) throws DocumentException {		
		ListItem item1 = new ListItem();
		Cell cell;

		Phrase item1Title = new Paragraph(defaultLeading, "Hydrocarbon Analysis", boldFont);
		item1.add(item1Title);		

		Table table = new Table(4);
		float[] widths = { 70, 40, 40, 40};
		table.setWidths(widths);
		table.setPadding(2);
		table.setAlignment(Element.ALIGN_LEFT);
		//Table heading
		Cell tableHeading = createCell("Extended Hydrocarbon Analysis");
		tableHeading.setHorizontalAlignment(Element.ALIGN_CENTER);
		tableHeading.setBorder(0);
		tableHeading.setColspan(4);
		table.addCell(tableHeading);
		// Column Header
		cell = createCell("Component(mol%)");
		table.addCell(cell);
		cell = createCell("Gas");
		table.addCell(cell);
		cell = createCell("Oil/Condensate");
		table.addCell(cell);
		cell = createCell("Produced Water");
		table.addCell(cell);
		//Table Content
		for(HydrocarbonAnalysisPollutant hcp : facility.getHydrocarbonPollutantList()){
			String temp = hcp.getPollutantDesc();
			cell = createCell(defaultLeading, temp, data1Font);
			table.addCell(cell);

			String gas = null;
			if (hcp.getGas() != null) {
				gas = hcp.getGas().toString();
			}
			cell = createCell(defaultLeading, gas, data1Font);
			table.addCell(cell);
			...
			String oil = null;
			if (hcp.getOil() != null) {
				oil = hcp.getOil().toString();
			}
			cell = createCell(defaultLeading, oil, data1Font);
			table.addCell(cell);

			String producedWater = null;
			if (hcp.getProducedWater() != null) {
				producedWater = hcp.getProducedWater().toString();
			}
			cell = createCell(defaultLeading, producedWater, data1Font);
			table.addCell(cell);
		}
	...
	private ListItem generateHydrocarbonAnalysisSampleDetailTable(Facility facility) throws DocumentException {		
		ListItem item1 = new ListItem();
		Cell cell;
		...
		table.setAlignment(Element.ALIGN_LEFT);
		//Table heading
		Cell tableHeading = createCell("Hydrocarbon Analysis Sample Detail");
		tableHeading.setHorizontalAlignment(Element.ALIGN_CENTER);
		tableHeading.setBorder(0);
		tableHeading.setColspan(4);
		table.addCell(tableHeading);
		...
		table.addCell(cell);
		//Table Content
		for(HCAnalysisSampleDetailRow sampleDetailRow : facility.convertHydrocarbonAnalysisSampleDetail()){
			String temp = sampleDetailRow.getRowDesc();
			cell = createCell(defaultLeading, temp, data1Font);
			table.addCell(cell);
	
			String gas = null;
			if (sampleDetailRow.getGasValue() != null) {
				gas = Timestamp.class.equals(sampleDetailRow.getGasValue().getClass()) ? dateFormat1.format(sampleDetailRow.getGasValue())
						: sampleDetailRow.getGasValue().toString();
			}		
			cell = createCell(defaultLeading, gas, data1Font);
			table.addCell(cell);
			...
			if (sampleDetailRow.getOilValue() != null) {
				oil = Timestamp.class.equals(sampleDetailRow.getOilValue().getClass()) ? dateFormat1.format(sampleDetailRow.getOilValue())
						: sampleDetailRow.getOilValue().toString();
			}
			cell = createCell(defaultLeading, oil, data1Font);
			table.addCell(cell);
			...
			if (sampleDetailRow.getWaterValue() != null) {
				water = Timestamp.class.equals(sampleDetailRow.getWaterValue().getClass()) ? dateFormat1.format(sampleDetailRow.getWaterValue())
						: sampleDetailRow.getWaterValue().toString();
			}
			cell = createCell(defaultLeading, water, data1Font);
			table.addCell(cell);
		}
	...
	private ListItem generateHydrocarbonAnalysisOtherParts(Facility facility) throws DocumentException {		
		ListItem item1 = new ListItem();
		...
		generate2Columns(table, "AQD Emission Factor Group:", AqdEmissionFactorGroupDef.getData().getItems().getItemDesc(facility.getAqdEmissionFactorGroupCd()),
				false, null, null, false);
		
		Cell foorNotes;
		String temp;
		
		temp = "\u2022 Other Hexanes:  The term 'Other Hexanes' refers to the sum of all organic compounds with six carbon atoms that are not specifically requested on this spreadsheet (e.g. 2-methylpentane, 3-methylpentane, etc.).  Normal hexane, Benzene, and cyclohexane are specifically requested and should therefore be omitted from the sum of 'Other Hexanes'.";
		foorNotes = createCell(defaultLeading, temp, smallerNormal);
		foorNotes.setBorder(0);
		foorNotes.setColspan(4);
		table.addCell(foorNotes);

		temp = "\u2022 Other Heptanes:  The term 'Other Heptanes' refers to the sum of all organic compounds with seven carbon atoms that are not specifically requested on this spreadsheet (e.g. n-heptane, 3-ethylpentane, cycloheptane, etc.).  Methylcyclohexane and Toluene are specifically requested and should therefore be omitted from the sum of 'Other Heptanes'.";
		foorNotes = createCell(defaultLeading, temp, smallerNormal);
		foorNotes.setBorder(0);
		foorNotes.setColspan(4);
		table.addCell(foorNotes);
				
		temp = "\u2022 Other Octanes:  The term 'Other Octanes' refers to the sum of all organic compounds with eight carbon atoms that are not specifically requested on this spreadsheet (e.g. n-octane, 2,3-Dimethylhexane, cyclooctane, etc.).  Ethylbenzene, o-xylene, m-xylene, p-xylene, and 2,2,4-Trimethylpentane are specifically requested and should therefore be omitted from the sum of 'Other Octanes'.";		
		foorNotes = createCell(defaultLeading, temp, smallerNormal);
		foorNotes.setBorder(0);
		foorNotes.setColspan(4);
		table.addCell(foorNotes);
		
		temp = "\u2022 Nonanes:  The term 'Nonanes' refers to the sum of all organic compounds with nine carbon atoms (e.g. n-nonane, 2,2,3-Trimethylhexane, indane, etc.)";
		foorNotes = createCell(defaultLeading, temp, smallerNormal);
		foorNotes.setBorder(0);
		foorNotes.setColspan(4);
		table.addCell(foorNotes);
		
		temp = "\u2022 Xylenes:  The term 'Xylenes' refers to the sum of all isomers of xylene (i.e. o-xylene, m-xylene, and p-xylene)";
		foorNotes = createCell(defaultLeading, temp, smallerNormal);
		foorNotes.setBorder(0);
		foorNotes.setColspan(4);
		table.addCell(foorNotes);		
		
		temp = "\u2022 Decanes+:  The term 'Decanes+' refers to the sum of all organic compounds with ten or more carbon atoms (e.g., decane, undecane, dodecane, 3,4-Diethylhexane, etc.)";								
		foorNotes = createCell(defaultLeading, temp, smallerNormal);
		foorNotes.setBorder(0);
		foorNotes.setColspan(4);
		table.addCell(foorNotes);	
	...
	private ListItem generateHydrocarbonAnalysisDecanePropertiesTable(Facility facility) throws DocumentException {
		ListItem item1 = new ListItem();
		Cell cell;

		Table table = new Table(3);
		float[] widths = { 70, 40, 40 };
		table.setWidths(widths);
		table.setPadding(2);
		table.setAlignment(Element.ALIGN_LEFT);
		// Table heading
		Cell tableHeading = createCell("Decanes+ Sample Properties");
		tableHeading.setHorizontalAlignment(Element.ALIGN_CENTER);
		tableHeading.setBorder(0);
		tableHeading.setColspan(3);
		table.addCell(tableHeading);
		...
		// Table Content
		DecaneProperties decaneProperties = facility.getDecaneProperties();
		if (null == decaneProperties) {
			decaneProperties = new DecaneProperties();
		}

		// molecular wt. row
		cell = createCell(defaultLeading, DecaneProperties.MOLECULAR_WEIGHT_PROPERTY_LABEL, data1Font);
		table.addCell(cell);

		cell = createCell(defaultLeading, decaneProperties.getAvgMolecularWtOfOil(), data1Font);
		table.addCell(cell);

		cell = createCell(defaultLeading, decaneProperties.getAvgMolecularWtOfProducedWater(), data1Font);
		table.addCell(cell);

		// specific gravity row
		cell = createCell(defaultLeading, DecaneProperties.SPECIFIC_GRAVITY_PROPERTY_LABEL, data1Font);
		table.addCell(cell);

		cell = createCell(defaultLeading, decaneProperties.getSpecificGravityOfOil(), data1Font);
		table.addCell(cell);

		cell = createCell(defaultLeading, decaneProperties.getSpecificGravityOfProducedWater(), data1Font);
		table.addCell(cell);
	...
	private String formatFugitiveComponent(Integer obj) {
		return NumberFormat.getNumberInstance(java.util.Locale.US).format(obj);
	}
