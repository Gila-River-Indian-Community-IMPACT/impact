...
public class GeneralPieImage extends ReportImageBase implements Image {
    ...
    public final void draw(int width, int height) throws Exception {
        SimpleIdDef[] pts;
        if (getStartDt() == null) {
            pts = getReportService().retrieveActivePermitsByGeneralByType(permitType,
                    issuanceType, doLaas, userId, activityName, activityStatusCd);
        } else {
            if ("EXP".equalsIgnoreCase(reportType)) {
                pts = getReportService().retrieveExpiredPermitsByGeneralByType(getStartDt(),
                        getEndDt(), permitType, issuanceType, doLaaName);
                
            } else {
                pts = getReportService().retrieveIssuedPermitsByGeneralByType(getStartDt(),
                        getEndDt(), permitType, issuanceType, doLaaName);
            }
        }
        ...
        for (SimpleIdDef p : pts) {
            ...
            // TODO see what data is really needed in this pie chart.
            // the values for p.getDescription are 'Y' or 'N' for PTIO permits
            /*
            ...
            */
            String label = "No";
            if ("Y".equalsIgnoreCase(p.getDescription())) {
                label = "Yes";
            }
            cd.setLabel(label);
