...
public class SOPPermitStatusPieImage extends ReportImageBase implements Image {
    public SOPPermitStatusPieImage() {
        super();
    }
    ...
    public final void draw(int width, int height) throws Exception {
        ...
        for (PermitSOPData sopData : details) {
            SimpleIdDef sd = sdMap.get(sopData.getStatusStr());
            if (sd == null) {
                sd = new SimpleIdDef();
                sd.setId(1);
                sd.setDescription(sopData.getStatusStr());
                sdMap.put(sopData.getStatusStr(), sd);
            } else {
                sd.setId(sd.getId() + 1);
            }
        }
