...
<f:view>
  ...
  <af:form usesUpload="true">
     ...
     <f:subview id="defList">             
      <af:panelForm>
		<af:outputText value="#{defCatalog.label}" />
	  </af:panelForm> 
      
      <af:forEach var="record" varStatus="vsi" items="#{defCatalog.customRecord.cells}">           
	      <af:panelForm>
	      	...
	      	<af:inputText 
			    ...
			    readOnly="#{record.readOnly && !defCatalog.newCustomRecord}" 
			    tip="Number"  
			...
			<af:panelLabelAndMessage for="docUrl" rendered="#{record.headerText == 'Template Path' && (!empty record.value)}" >
				...
				<af:goLink targetFrame="_blank" destination="#{record.docURL}"
					disabled="#{!record.documentTemplateExists}"
			...
			<af:inputText 
				label="#{record.headerText}" 
			    value="#{record.value}" 
			    rendered="#{!record.pickList && record.dataType != 'TIMESTAMP(6)' && record.dataType != 'date' && record.dataType != 'NUMBER' && record.dataType != 'numeric' && record.headerText != 'Template Path'}" 
			    maximumLength="#{record.maximumLength}" 
			    readOnly="#{record.readOnly && !defCatalog.newCustomRecord}" 
			    tip="Text"  
			    showRequired="#{record.required}"
			...
			<af:selectInputDate 
				label="#{record.headerText}" 
			    value="#{record.value}" 
			    rendered="#{!record.pickList && (record.dataType == 'TIMESTAMP(6)' || record.dataType == 'date')}" 
			    readOnly="#{record.readOnly && !defCatalog.newCustomRecord}" 
			    tip="Date"  
			    showRequired="#{record.required}"
			    required="#{record.required}">
          ...
          <afh:rowLayout halign="center">           
           <af:panelButtonBar>
            <af:commandButton text="Save"
                actionListener="#{defCatalog.saveCustom}"/>
