...
<af:table value="#{companyProfile.notesCmpWrapper}" bandingInterval="1"
	binding="#{companyProfile.notesCmpWrapper.table}" id="NotesTab"
	banding="row" width="98%" var="note" rows="#{companyProfile.pageLimit}">
	<af:column sortProperty="c01" sortable="true" formatType="text"
		...
		<af:commandLink text="#{note.noteId}" id="viewNote" useWindow="true"
			...
			disabled="#{companyProfile.editable}"
			returnListener="#{companyProfile.dialogDone}"
			action="#{companyProfile.startViewNote}">
			<t:updateActionListener property="#{companyProfile.modifyCompanyNote}"
				value="#{note}" />
		</af:commandLink>
	</af:column>
		...
		<afh:rowLayout halign="center">
			<af:panelButtonBar>
				<af:commandButton text="Add Note" id="AddNoteButton"
					...
					disabled="#{companyProfile.readOnlyUser or companyProfile.generalUser or companyProfile.releasePlannerUser}"
					returnListener="#{companyProfile.dialogDone}"
