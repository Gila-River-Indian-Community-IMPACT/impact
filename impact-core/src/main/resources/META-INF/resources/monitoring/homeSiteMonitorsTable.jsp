...
<af:table value="#{homeMonitorSiteDetail.siteMonitorsWrapper}" bandingInterval="1"
	binding="#{homeMonitorSiteDetail.siteMonitorsWrapper.table}" id="SiteMonitorsTab"
	banding="row" width="98%" var="monitor"
	rows="#{homeMonitorSiteDetail.pageLimit}">
	<af:column sortProperty="mntrId" sortable="true" formatType="text"
		headerText="Monitor ID">
		<af:commandLink action="#{homeMonitorDetail.submitFromAmbientJsp}">
			<af:outputText value="#{monitor.mntrId}" />
			<t:updateActionListener
				property="#{homeMonitorDetail.monitorId}"
	...
	<af:column sortProperty="type" sortable="true" formatType="text"
		...
		<af:selectOneChoice value="#{monitor.type}" readOnly="true">
			<f:selectItems value="#{homeMonitorDetail.typeDefs.allItems}" />
		</af:selectOneChoice>
	</af:column>
	...
	<af:column sortProperty="status" sortable="true" formatType="text"
		...
		<af:selectOneChoice value="#{monitor.status}" readOnly="true">
			<f:selectItems value="#{homeMonitorDetail.statusDefs.allItems}" />
		</af:selectOneChoice>
	</af:column>
