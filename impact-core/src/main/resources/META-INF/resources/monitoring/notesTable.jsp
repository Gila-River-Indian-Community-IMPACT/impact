...
<af:table value="#{monitorGroupDetail.notesWrapper}" bandingInterval="1"
	binding="#{monitorGroupDetail.notesWrapper.table}" id="NotesTab"
	...
	<af:column sortProperty="c01" sortable="true" formatType="text"
		...
		<af:commandLink text="#{note.noteId}" id="viewNote" useWindow="true"
			...
			action="#{monitorGroupDetail.startViewNote}">
			<t:updateActionListener property="#{monitorGroupDetail.modifyNote}"
				value="#{note}" />
		</af:commandLink>
	</af:column>
		...
		<afh:rowLayout halign="center">
			<af:panelButtonBar>
				<af:commandButton text="Add Note" id="AddNoteButton"
					...
					disabled="false" returnListener="#{monitorGroupDetail.dialogDone}"
					rendered="#{!monitorGroupDetail.readOnlyUser}" action="#{monitorGroupDetail.startAddNote}">
				</af:commandButton>
