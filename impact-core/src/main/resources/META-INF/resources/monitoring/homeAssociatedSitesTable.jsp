...
<af:table value="#{homeMonitorGroupDetail.associatedSitesWrapper}" bandingInterval="1"
	binding="#{homeMonitorGroupDetail.associatedSitesWrapper.table}" id="AssociatedSitesTab"
	...
	<af:column sortProperty="mstId" sortable="true" formatType="text"
		headerText="Site ID">
		<af:commandLink action="#{homeMonitorSiteDetail.submitFromAmbientJsp}">
			<af:outputText value="#{site.mstId}" />
			<t:updateActionListener
				property="#{homeMonitorSiteDetail.siteId}"
				value="#{site.siteId}" />
		</af:commandLink>
	</af:column>
		...
		<afh:rowLayout halign="center">
			<af:panelButtonBar>
				<af:commandButton text="Add Site" id="AddSiteButton"
					...
					disabled="#{monitorGroupSearch.readOnlyUser}"
					returnListener="#{homeMonitorGroupDetail.associatedSitesDialogDone}" 
					rendered="#{monitorGroupSearch.internalApp && !monitorGroupSearch.readOnlyUser}"
					action="#{homeMonitorGroupDetail.startAddSite}">
				</af:commandButton>
