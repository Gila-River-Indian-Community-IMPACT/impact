...
<af:table value="#{monitorDetail.notesWrapper}" bandingInterval="1"
	binding="#{monitorDetail.notesWrapper.table}" id="NotesTab"
	banding="row" width="98%" var="note"
	rows="#{monitorDetail.pageLimit}">
	<af:column sortProperty="c01" sortable="true" formatType="text"
		...
		<af:commandLink text="#{note.noteId}" id="viewNote" useWindow="true"
			...
			returnListener="#{monitorDetail.dialogDone}"
			action="#{monitorDetail.startViewNote}">
			<t:updateActionListener property="#{monitorDetail.modifyNote}"
				value="#{note}" />
		</af:commandLink>
	</af:column>
		...
		<afh:rowLayout halign="center">
			<af:panelButtonBar>
				<af:commandButton text="Add Note" id="AddNoteButton"
					...
					disabled="false" returnListener="#{monitorDetail.dialogDone}"
					rendered="#{!monitorDetail.readOnlyUser}" action="#{monitorDetail.startAddNote}">
				</af:commandButton>
