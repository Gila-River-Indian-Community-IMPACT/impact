				...
				<afh:rowLayout halign="center">
					<af:panelButtonBar>
						<af:commandButton text="Edit"
							rendered="#{facilityProfile.dapcUser && !facilityProfile.facilityCemComLimitEditMode}"
							disabled="#{!facilityProfile.facilityCemComLimitEditAllowed || facilityProfile.disabledUpdateButton}"
							action="#{facilityProfile.editFacilityCemComLimit}" />
						<af:commandButton text="Save" rendered="#{facilityProfile.facilityCemComLimitEditMode}"
							action="#{facilityProfile.saveFacilityCemComLimit}" />
						<af:commandButton text="Cancel"
							immediate="true"
							rendered="#{facilityProfile.facilityCemComLimitEditMode}"
							action="#{facilityProfile.cancelFacilityCemComLimit}" />
						<af:commandButton text="Delete"
							rendered="#{facilityProfile.dapcUser && facilityProfile.stars2Admin && !facilityProfile.facilityCemComLimitEditMode}"
							disabled="#{!facilityProfile.facilityCemComLimitDeleteAllowed}"
							useWindow="true" windowWidth="#{confirmWindow.width}"
							windowHeight="#{confirmWindow.height}"
							returnListener="#{facilityProfile.deleteFacilityCemComLimit}"
							action="#{confirmWindow.confirm}"
							shortDesc="#{facilityProfile.facilityCemComLimitDeleteAllowed ? 'Delete' : 
	                			facilityProfile.facilityCemComLimitDeleteAllowedMsg}">
							...
							<t:updateActionListener property="#{confirmWindow.message}"
								value="Are you sure you want to delete limit #{facilityProfile.modifyFacilityCemComLimit.limId}?" />
						</af:commandButton>
						<af:commandButton text="Close"
							immediate="true"
							rendered="#{!facilityProfile.facilityCemComLimitEditMode}"
							action="#{facilityProfile.closeFacilityMonitorCemComLimitDialog}" />
					</af:panelButtonBar>
				</afh:rowLayout>
			</af:panelForm>
		</af:form>
