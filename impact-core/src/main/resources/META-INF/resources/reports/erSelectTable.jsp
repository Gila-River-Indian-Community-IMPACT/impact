...
<af:table value="#{reportSearch.reports}" bandingInterval="1"
	banding="row" var="reportResults" rows="#{reportSearch.pageLimit}">
	<af:column sortable="true" sortProperty="selected" formatType="icon"
		headerText="Select">
		<af:selectBooleanRadio group="RadioButtons" rendered="true"
			disabled="#{reportProfile.reportId==reportResults.emissionsRptId}"
			value="#{reportResults.selected}">
	...
	<af:column sortProperty="emissionsInventoryId" sortable="true"
		...
		<af:outputText value="#{reportResults.emissionsInventoryId}"
				inlineStyle="#{reportProfile.reportId==reportResults.emissionsRptId?'font-weight:bold':''}"/>
	</af:column>
	...
	<%--
	  ...
	  --%>
	<af:column sortProperty="totalReportedEmissionsDigits" sortable="true"
		headerText="Total Emissions Reported (Tons)" formatType="number">
