...
<af:column headerText="Select" sortProperty="c00" sortable="#{!reportProfile.doRptCompare && !reportProfile.editable}" formatType="icon"
	rendered="#{reportProfile.editable && !reportProfile.expandedExp}">
	<af:selectBooleanCheckbox rendered="#{emissionLine.deletable}"
		value="#{emissionLine.delete}" />
</af:column>
...
<af:column sortProperty="pollutantCd" sortable="#{!reportProfile.doRptCompare && !reportProfile.editable}"
...
<af:column headerText="Hazardous Air Pollutants/Greenhouse Gases/Other" sortable="false" sortProperty="haps"
	rendered="#{!reportProfile.expandedExp}" width="450">
	<af:column sortProperty="pollutantCd" sortable="#{!reportProfile.doRptCompare && !reportProfile.editable}"
	...
	<af:column sortProperty="pollutantCd" sortable="#{!reportProfile.doRptCompare && !reportProfile.editable}" rendered="false" 
		headerText="Code" formatType="text" width="50">
		<af:outputText value="#{emissionLine.pollutantCd}" truncateAt="50" shortDesc="#{emissionLine.pollutantCd}"/>
	</af:column>
	<af:column sortProperty="category" sortable="#{!reportProfile.doRptCompare && !reportProfile.editable}" rendered="false"
		headerText="Type" formatType="icon" width="100">
		<af:outputText value="#{emissionLine.category}" />
	</af:column>
</af:column>
...
<af:column sortProperty="emissionCalcMethodCd" sortable="#{!reportProfile.doRptCompare && !reportProfile.editable}"
	formatType="text" headerText="Method Used"
	rendered="#{!reportProfile.renderSum && !reportProfile.expandedExp}" width="250">
	...
	<af:commandLink useWindow="true" windowWidth="1200" windowHeight="700"
		...
		<af:selectOneChoice value="#{emissionLine.emissionCalcMethodCd}"
			...
			readOnly="#{!reportProfile.editable}" id="methodIdReadOnly" autoSubmit="true"
	...
	<af:outputText value="expired WebFIRE factor"
		...
		inlineStyle="color: orange; font-weight: bold;"
		shortDesc="WebFire factor no longer active.  Please select a different WebFire factor or a different calculation method.">
	</af:outputText>
...
<af:column formatType="number" sortProperty="factorNumericValue"
	...
	<af:outputText value="pending variable amount" inlineStyle="color: orange; font-weight: bold;"
		rendered="#{(emissionLine.fireRef != null && emissionLine.factorNumericValue == null && ! reportProfile.editable) && !reportProfile.doRptCompare}"
		shortDesc="Factor formula requires a variable. The formula variables have not been given values">
	</af:outputText>
	<af:outputText value="pending" inlineStyle="color: orange; font-weight: bold;"
		shortDesc="Factor to determine emissions prior to controls"
		rendered="#{(emissionLine.fireRef == null && !reportProfile.renderSum && ! reportProfile.editable && emissionLine.factorNumericValue==null && emissionLine.emissionCalcMethodCd < 200) && !reportProfile.doRptCompare}">
	</af:outputText>
</af:column>
...
<af:column rendered="false" headerText="Trade Secret" sortable="true" sortProperty="tradeSecretF">
	<afh:rowLayout>
...
<af:column headerText="Emissions Reported"
	rendered="#{!reportProfile.expandedExp}" sortable="false" sortProperty="emissionsReported"
	width="280">
	...
	<af:column sortProperty="emissionsUnitNumerator" sortable="#{!reportProfile.doRptCompare && !reportProfile.editable}"
		formatType="icon"  headerText="Units" width="10">
...
<af:column rendered="false" headerText="Trade Secret" sortable="true" sortProperty="tradeSecretE">
	<afh:rowLayout>
		<af:selectBooleanCheckbox readOnly="#{true}"
