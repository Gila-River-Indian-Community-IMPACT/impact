    ...
    <af:table value="#{attachments.attachmentList}" bandingInterval="1"
      ...
      <af:column sortable="true" sortProperty="documentID" noWrap="true"
        ...
        <af:commandLink 
          ...
          rendered="#{(stackTestDetail.internalApp && !stackTestDetail.readOnlyUser) || (stackTestDetail.portalApp && !stackTestDetail.viewOnly)}"
          ...
          <t:updateActionListener property="#{attachments.locked}" 
          	value="#{stackTestDetail.stackTest.submittedDateLong < attachments.tempDoc.lastModifiedTSLong ? false : stackTestDetail.locked}" />
        </af:commandLink>
        <af:outputText value="#{report.documentID}" 
          rendered="#{(stackTestDetail.internalApp && stackTestDetail.readOnlyUser) || (!stackTestDetail.internalApp && stackTestDetail.viewOnly)}"/>
      </af:column>
      ...
      <af:column sortable="true" sortProperty="attachmentType"
        ...
        <af:goLink inlineStyle="clear:none" targetFrame="_blank"
          ...
          disabled="#{stackTestDetail.editable}" />
      </af:column>
	  ...
	  <af:column headerText="Trade Secret Document"
		...
		<af:switcher defaultFacet="noTS"
			...
			<f:facet name="TS">
			  <af:switcher defaultFacet="downloadTSDoc" facetName="#{(stackTestDetail.internalApp)? 'downloadTSDoc' : 'noDownloadTSDoc'}">
			    <f:facet name="noDownloadTSDoc">
				  <af:goLink
					id="tradeSecretDocDownloadLink" text="Download" disabled="#{stackTestDetail.editable}" 
					targetFrame="_blank" destination="#{report.tradeSecretDocURL}"
				...
				<f:facet name="downloadTSDoc">
					...
					disabled="#{stackTestDetail.editable}" useWindow="true"
					windowWidth="800" windowHeight="400"
					shortDesc="Download trade secret document (#{(empty report) ? '' : report.extension})">
				</af:commandLink>
				</f:facet>
				...
				<af:switcher defaultFacet="noTSDoc"
					...
					<f:facet name="TSDoc">
						<af:commandLink text="Add Justification"
							...
							<t:updateActionListener property="#{attachments.locked}" value="#{stackTestDetail.locked}" />
						</af:commandLink>
					</f:facet>
