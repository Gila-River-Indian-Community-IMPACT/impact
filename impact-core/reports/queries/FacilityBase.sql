SELECT ff.fp_id, ff.facility_id, ff.version_id, facility_nm, ff.operating_status_cd, ff.emissions_rpt_cd, 
    ff.permit_status_cd, ff.permit_classification_cd, ff.last_modified AS facility_lm, 
    ca.address_id, ca.address_type_cd, ca.address1, ca.address2, ca.city, ca.state_cd, ca.zip5, 
    ca.zip4, ca.country_cd, ca.county_cd, 
      ...
      cm_county_def ccd, cm_state_def csd, cm_address_type_def cat, cm_do_laa_def fpdo  
      WHERE ff.version_id = -1
	  AND ff.fp_id = faxr.fp_id AND faxr.address_id = ca.address_id
	  AND ca.end_dt is null AND ca.address_type_cd = cat.address_type_cd
	  AND ca.county_cd = ccd.county_cd AND ccd.state_cd = csd.state_cd
	  AND ff.do_laa_cd = fpdo.do_laa_cd  
          ...
          AND LOWER(EMISSIONS_RPT_CD) LIKE LOWER($P{REPORT_CATEGORY_CD})